document.addEventListener('DOMContentLoaded', () => {
    ...
    function searchWords(term) {
        return new Promise((resolve) => {
            ...
            // 1. 영어 인덱스 검색 (starts with)
            const engIndex = store.index("english_idx");
            const engRange = IDBKeyRange.bound(searchTerm, searchTerm + '\uffff');
            engIndex.openCursor(engRange).onsuccess = e => {
                const cursor = e.target.result;
                if (cursor) {
                    results.set(cursor.value.id, cursor.value);
                    cursor.continue();
                } else {
                    onQueryComplete();
                }
    ...
    function showDetailView(word) { currentWord = word; flashcardEng.textContent = word.english; flashcardKor.textContent = word.korean; flashcard.classList.remove('flipped'); updateFavoriteButton(); showView('detail'); flashcardFront.scrollTop = 0; flashcardBack.scrollTop = 0; }
    ...
    // ----- 앱 초기화 -----
    async function init() { applyFontSize(); try { await openDB(); const isDataReady = await checkDBStatus(); if (isDataReady) showView('main'); else showView('setup'); } catch (error) { alert("앱 초기화 오류: " + error); document.body.innerHTML = "<h1>앱 로딩 실패</h1>"; } }
